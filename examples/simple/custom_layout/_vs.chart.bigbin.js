...
$.fn._vs.chart.bigbin = function(_this,fn,options) {
  ...
  this.setupChartPhysics = function(_this){
    ...
    if(getURLParameter("tokenMin")!="null"){
      _this.settings.sedimentation.token.size.minimum = getURLParameter("tokenMin")
    }  
    if(getURLParameter("tokenSize")!="null"){
  ...
  this.token = function (_this,options){
    ...
    var token = {
              ...
              category:i,
              fillStyle: _this.token.colorRange(i),
              targets:[]
            }
    return token; 
  }
  ...
  this.createBoxContainer  = function (_this,x,y,w,h,l,r,slope,outSize,color){
    ...
    this.createMyChartBox ( _this,x,y,l,h,null,"wall",color);
    this.createMyChartBox ( _this,x+w,y,l,h,null,"wall",color);
    this.createMyChartBox ( _this,x,y+h,(w/2)-(outSize/2),l,slope1,"wall",color);
    this.createMyChartBox ( _this,x+w-(w/2)+(outSize),y+h,(w/2)-(outSize/2),l,slope2,"wall",color);
